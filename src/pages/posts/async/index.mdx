...
which allows it to update its own state so it can make progress.
...
The problem then becomes how you interweave those tasks, and how you know
...
Ok, [looking at the docs](https://doc.rust-lang.org/std/task/struct.Context.html) we see
...
Ok, so `Context` is related to an async task, but it seems this `Waker` thing is more important.
...
Ok ok, as promised, here is async code in its full glory.
...
Hmm, well, we're intending them to borrow from `buf` and `reader` inside the future itself...
This seems self referential. [Self referential structs are _hard_](https://rust-unofficial.github.io/too-many-lists/fourth.html)
    ...
    fn poll(&mut self) -> Poll<Self::Output> {
        match self {
            ...
            Reader { buf, reader } => {
                ...
                unsafe {
                    // lifetime extension
                    let buf2 = &mut *(buf as *mut _);
                    let reader2 = &mut *(reader as *mut _);
