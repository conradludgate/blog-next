...
fn main() {
    ...
    let n = loop {
        ...
        match fut.poll() {
            ...
            Poll::Pending => println!("value was not ready - {fut:?}"),
        }
    };
    // Done!
...
Ok ok, as promised, here is async code in it's full glory.
...
We can see here that we await for the value of `step1`, and then we multiply it by 7 and return it.
...
Right, okay. So we can use unsafe to break our references a bit, make them static or something.

```rust
// Safety:
// Our caller should not move our future (and therefore our buf)
let buf = unsafe {
    core::mem::transmute::<&mut [u8], &'static mut [u8]>(&mut buf);
};
```

That's great, but how do we tell our callers about these safety guarantees?
I guess we could make the `poll` method unsafe, but that kinda sucks since this
makes every single future unsafe to call, and removes any flexibility on what
these safety guarantees are.

Well, maybe there is another way?
What if we could make it unsafe to move the value _at all_ after we call `poll`?

That's what `Pin` is! `Pin` is a type that's unsafe to construct,
unless our type implements `Unpin` (an trait that guarantees it's not self-referential),
or unless you use the `Box::pin` method to pin it in a boxed allocation (since boxed data is guaranteed to not move).
...
So, we need a way to tell our executor that this sleep is unlikely to make progress.
Ideally, all tasks would tell the executor that they are ready for more work. This is where
our Waker comes in.
...
Which enqueues the task just as soon as it returns pending. What if we instead got the task
itself to do this for us.

When our task returns pending, instead of giving it back to the executor queue, we will give
it to the context. And the task will put the context onto some scheduler that will then
call the `wake` method, this will remove the task from the context and put it back in the work queue
