...
test("challenge conditions require a sustained observation and remain complete", () => {
	const conditions = [{
		id: "overload",
		label: "Overloaded",
	...
	progress = updateConditionProgress(conditions, progress, { ...baseline, nowMs: 0, queueDepth: 4 }, baseline);
	assert.equal(progress.overload.completedAtMs, null);
	progress = updateConditionProgress(conditions, progress, { ...baseline, nowMs: 500, queueDepth: 4 }, baseline);
	assert.equal(progress.overload.completedAtMs, null);
	progress = updateConditionProgress(conditions, progress, { ...baseline, nowMs: 750, queueDepth: 0 }, baseline);
	assert.equal(progress.overload.activeSinceMs, null, "a brief healthy tick resets the sustained observation");
	progress = updateConditionProgress(conditions, progress, { ...baseline, nowMs: 1000, queueDepth: 4 }, baseline);
	progress = updateConditionProgress(conditions, progress, { ...baseline, nowMs: 2000, queueDepth: 4 }, baseline);
	assert.equal(progress.overload.completedAtMs, 2000);
	progress = updateConditionProgress(conditions, progress, { ...baseline, nowMs: 2250, queueDepth: 0 }, baseline);
	assert.equal(progress.overload.completedAtMs, 2000, "a completed lesson check is never undone by a later transient");
});

test("challenge predicates remain incomplete before the reader makes the required change", () => {
	const conditions = [{
		id: "changed",
		label: "Changed",
	...
	const baseline = createInitialState();
	const unchanged = { ...baseline, nowMs: 9750 };
	const changed = { ...baseline, nowMs: 10_000, serviceMs: 3000 };
	let progress = updateConditionProgress(conditions, createConditionProgress(conditions), unchanged, baseline);
	assert.equal(progress.changed.completedAtMs, null);
	progress = updateConditionProgress(conditions, progress, changed, baseline);
	assert.equal(progress.changed.completedAtMs, 10_000);
});

test("challenge conditions can require a change from the initial baseline", () => {
	const conditions = [{
		id: "growth",
		label: "Queue grew",
	...
	let progress = createConditionProgress(conditions);
	progress = updateConditionProgress(conditions, progress, unchanged, baseline);
