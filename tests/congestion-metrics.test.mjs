...
test("saturated completion rate matches capacity including dispatch and tick rounding", () => {
	let state = setClientCount(setWorkerCount(createInitialState("concurrency"), 1), 8);
	state.performanceJitter = 0;
...
test("rejections trigger backoff without becoming successful RTT samples", () => {
	for (const strategy of ["vegas", "gradient2"]) {
		...
		state.jobs = Array.from({ length: 6 }, (_, id) => ({ id, client: 0, service: 0, stage: "queue", remainingMs: 0, createdAt: 0 }));
...
test("display metrics weight requests rather than clients or ticks", () => {
	const metrics = summarizeMetrics([
		{ atMs: 250, sent: 10, completed: 9, rejected: 0, latencySumMs: 9000, latencySamplesMs: Array(9).fill(1000), workerBusy: [true, false] },
		{ atMs: 500, sent: 0, completed: 0, rejected: 1, latencySumMs: 0, latencySamplesMs: [], workerBusy: [true, true] },
		{ atMs: 750, sent: 0, completed: 1, rejected: 0, latencySumMs: 3000, latencySamplesMs: [3000], workerBusy: [false, false] },
	], 10000);
	...
	assert.equal(metrics.p50LatencyMs, 1000);
	assert.equal(metrics.p99LatencyMs, 3000);
	assert.equal(metrics.utilisation, 0.5);
});
