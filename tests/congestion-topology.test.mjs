...
test("each worker has a bounded FIFO; idle neighbours cannot steal queued work", () => {
...
test("sampled endpoints are distinct, including the singleton case", () => {
	...
	assert.deepEqual(sampleTwo(1, random), [0]);
	for (let i = 0; i < 100; i++) {
		const pair = sampleTwo(8, random);
		assert.notEqual(pair[0], pair[1]);
		assert.ok(pair.every(worker => worker >= 0 && worker < 8));
	}
});

test("simulation randomness is reproducible from state", () => {
	let first = createInitialState("vegas", 42);
	let replay = createInitialState("vegas", 42);
	let different = createInitialState("vegas", 43);
	for (let tick = 0; tick < 100; tick++) {
		first = advanceSimulation(first);
		replay = advanceSimulation(replay);
		different = advanceSimulation(different);
	}
	assert.deepEqual(replay, first);
	assert.notDeepEqual(different.workerPerformance, first.workerPerformance);
});

test("an idle worker accepts one active request plus its bounded waiting queue", () => {
	let state = setWorkerCount(createInitialState("rate"), 1);
	state.jobs = Array.from({ length: WORKER_QUEUE_LIMIT + 2 }, (_, index) => job(100 + index, 0));
	state.nextJobId = 200;
	state = advanceSimulation(state);
	assert.equal(state.jobs.filter(candidate => candidate.stage === "serviceDispatch").length, 1);
	assert.equal(state.jobs.filter(candidate => candidate.stage === "queue").length, WORKER_QUEUE_LIMIT);
	assert.equal(state.dropped, 1);
});

test("blocked demand retains its candidates instead of resampling idle alternatives", () => {
...
test("success and rejection update only the originating client–worker controller", () => {
	let state = setClientCount(createInitialState("vegas"), 2);
	state.clients[0].endpoints[0].controller.limit = 8;
	state.jobs = Array.from({ length: 6 }, (_, i) => job(100 + i, 0));
...
test("all strategies keep valid destinations and bounded queues through fleet changes", () => {
